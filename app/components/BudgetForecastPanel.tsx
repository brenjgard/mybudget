    ...
    <section className="rounded-2xl border border-harbor-teal-light bg-white shadow-sm">
      <div className="border-b border-harbor-teal-light px-4 py-4">
        <p className="text-xs font-semibold uppercase tracking-wide text-harbor-teal">
          {isPlan ? "Monthly Budget Plan" : "Spending Tracker"}
        </p>
        <h2 className="mt-1 text-xl font-bold text-harbor-navy">
          {isPlan ? "What This Month Is Expected to Cost" : "Are We Staying on Budget?"}
        </h2>
      </div>
        ...
        <table className="w-full min-w-[760px] border-collapse text-sm">
          <thead>
            <tr className="bg-harbor-teal-light text-harbor-navy">
              ...
              {isPlan ? (
                <>
                  <th className="px-3 py-3 text-right">Planned Checking/Cash</th>
                  <th className="px-3 py-3 text-right">Planned Credit Card</th>
                </>
              ) : (
                <>
          ...
          <tbody>
            {forecast.categories.map((category) => (
              <tr key={category.categoryId} className="border-b border-slate-100">
                ...
                {isPlan ? (
                  <>
                    <td className="px-3 py-3 text-right text-harbor-navy">{formatMoney((category.plannedByPaymentMethod.checking ?? 0) + (category.plannedByPaymentMethod.cash ?? 0))}</td>
                    <td className="px-3 py-3 text-right text-harbor-navy">{formatMoney(category.plannedByPaymentMethod.credit_card ?? 0)}</td>
                  </>
                ) : (
                  <>
